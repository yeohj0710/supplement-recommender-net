# train_export_c_section_onnx.py
...
class CSDataset(Dataset):
    def __init__(self, csv_path, cat_order):
        ...
        Xv = self.df[["v1", "v2", "v3", "v4", "v5"]].values.astype(np.float32)
...
class CSNet(nn.Module):
    ...
    def forward(self, cat_ids, answers):
        a = answers.to(dtype=torch.float32)
        a = torch.clamp(a, 0.0, 1.0)
        e = self.emb(cat_ids)
        x = torch.cat([a, e], dim=1)
...
def export_onnx(model, out_path, cat_order):
    ...
    with torch.no_grad():
        ...
        dummy_x = torch.tensor([[0.0] * 5, [1.0] * 5, [0.5] * 5], dtype=torch.float32)
        torch.onnx.export(
            model,
            (dummy_c, dummy_x),
            out_path,
